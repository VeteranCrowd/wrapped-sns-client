...
export class WrappedSnsClient {
   ...
   *
   * @param {object} record - CRUD record.
   ...
   * @return {Promise<string>} Message ID.
   */
  async publishCrud2Topic(record, topicArn) {
    // Validate parameters.
    if (!_.isPlainObject(record)) throw new Error('invalid record');
    if (!_.isString(topicArn) || !topicArn.length)
      throw new Error('invalid topicArn');

    // Extract attributes.
    const [entity] =
      record.dynamodb?.Keys?.entityPK?.S?.match(/^\w+(?=!?)/) ?? [];
    const operation = record.eventName.toLowerCase();

    // Publish record.
    return await this.publish({
      attributes: { entity, operation },
      message: record,
      topicArn,
    });
  }
